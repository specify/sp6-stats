$(function () {
  ...
  function redirect() {
    ...
    if (isNaN(date1_val)) date1_val = '';

    if (isNaN(date2_val)) date2_val = '';

    if (date1_val > date2_val) [date1_val, date2_val] = [date2_val, date1_val];

    window.location.href =
      link + 'stats/?' + get_dates() + get_search() + get_institution();
  }
  ...
  //Construct URLs
  function get_search() {
    if (search_query !== '') return 'search_query=' + search_query + '&';
    return '';
  }
