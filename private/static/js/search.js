$(function () {
  ...
  $.each($('ol > li'), function (index, element) {
    //institutions
    ...
    $.each(institution.find('> ul > li'), function (index, element) {
      //disciplines
      ...
      $.each(discipline.find('> ul > li'), function (index, element) {
        //collections
  ...
  function update_stats(defaults = []) {
    ...
    if (defaults.length === 4)
      ...
      $.each(
        institutions,
        function (index, [institution, institution_name, disciplines]) {
          $.each(
            disciplines,
            function (index, [discipline, discipline_name, collections]) {
              $.each(
                collections,
                function (
                  index,
                  [collection, collection_name, reports_count_local]
                ) {
              ...
              if (!discipline.hasClass('greyed_out')) disciplines_count++;
            }
          );

          if (!institution.hasClass('greyed_out')) institutions_count++;
        }
      );
    }
  ...
  function search() {
    ...
    if (search_query === '') {
      ...
      update_stats([
        initial_institutions_count,
        initial_disciplines_count,
        initial_collections_count,
          ...
          function (index, [institution, institution_name, disciplines]) {
            ...
            $.each(
              ...
              function (index, [discipline, discipline_name, collections]) {
                ...
                if (discipline_matches) disciplines_match = true;
                ...
                $.each(
                  ...
                  function (
                    ...
                    const collection_matches =
                      collection_name.match(regex) !== null;

                    if (collection_matches) collections_match = true;
                    else collection.addClass('greyed_out');
                  }
                );
